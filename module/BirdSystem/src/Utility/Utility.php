...
class Utility
{
     ...
     */
    public static function arrayInWhere($array, $isInt = false)
    {
        if ($isInt) {
            return new Expression(implode(',', $array));
        } else {
            return new Expression("'" . implode("','", $array) . "'");
        }
    }
    ...
    /**
     * This method is only intended to be used in CronJob,
     * where no user is logged in but we need to predent it's there.
     *
     * @param UserInfo       $UserInfo
     * @param string         $module
     * @param ServiceManager $ServiceLocator
     *
     * @throws \BirdSystem\Exception
     */
    // @formatter:off
    public static function fakeAuthService(
        UserInfo $UserInfo,
        ServiceManager $ServiceLocator,
        $module = AuthenticationServiceFactory::MODULE_ADMIN
    )
    {
        // @formatter:on
        $AuthenticationServiceFactory = new AuthenticationServiceFactory();
        $AuthService                  = $AuthenticationServiceFactory->create($module);
        $AuthService->getStorage()->write($UserInfo);
        $ServiceLocator->setService('AuthService', $AuthService);
    }
