...
use BirdSystem\Db\TableGateway\Config;
...
class Measure implements LoggerAwareInterface
{
    ...
    public function init()
    {
        ...
        $configCode = [
            Config::CONFIG_DEFAULT_VOLUME_UNIT,
            Config::CONFIG_DEFAULT_WEIGHT_UNIT,
            Config::CONFIG_DEFAULT_LENGTH_UNIT,
        ];

        // Set user unit
        if ($UserInfo->isClientUserInfo()) {
            $ClientConfigTableGateway =
                $this->serviceLocator->get(ClientConfig::class);
            $config                   = $ClientConfigTableGateway->getValueByCodes($configCode);
        } else {
            $CompanyConfigTableGateway =
                $this->serviceLocator->get(CompanyConfig::class);
            $config                    = $CompanyConfigTableGateway->getValueByCodes($configCode);
        }

//        $this->log('User Measure Unit', $config);

        $this->userLengthUnit = $config[Config::CONFIG_DEFAULT_LENGTH_UNIT];
        $this->userWeightUnit = $config[Config::CONFIG_DEFAULT_WEIGHT_UNIT];
        $this->userVolumeUnit = $config[Config::CONFIG_DEFAULT_VOLUME_UNIT];
